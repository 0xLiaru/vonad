    ...
    return (
      <aside className="w-72 border-r border-slate-600/50 bg-slate-900/60 flex flex-col shrink-0">
        <div className="p-3 border-b border-slate-700/30 flex items-center gap-2">
          <button onClick={clearTopic} className="text-white hover:text-purple-400 transition-colors p-1" title="Geri">
            <ArrowLeft size={16} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-3">
          ...
          <p className="text-white text-xs font-semibold uppercase tracking-wider mb-2">Bloklar</p>
  ...
  return (
    <aside className="w-72 border-r border-slate-600/50 bg-slate-900/60 flex flex-col shrink-0">
      <div className="p-3 border-b border-slate-700/30">
        <p className="text-white font-bold text-sm mb-2">Konular</p>
          ...
          return (
            <button key={key} onClick={() => selectTopic(key)}
              ...
              <div className="flex items-center gap-2 mt-1.5 ml-12">
                ...
                {isFree(key) && (
                  <span className="text-green-300 text-[10px] px-1.5 py-0.5 rounded-full bg-green-500/20 border border-green-500/30">Ucretsiz</span>
                )}
              </div>
            </button>
          )
        })}
        {filteredTopics.length === 0 && <p className="text-slate-400 text-sm text-center py-8">Konu bulunamadi</p>}
      </div>
    </aside>
  )
}
